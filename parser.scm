...
(define flat_seq_helper2
  (lambda (exp)
    (cond 
    	((or (null? exp) (atom? exp))
    		exp)
    	((member (car exp) 
    			 '(if3 def lambda-simple lambda-opt lambda-var applic set or))
    		exp)
    	...
    	((and (list? (car exp))
    	 	  (not (null? (car exp))) 
    		  (equal? (caar exp) 'seq))
    	   ;`(,@(flat_seq_helper2 (cadar exp)) ,@(flat_seq_helper2 (cdr exp)) ))
    	   (append (flat_seq_helper2 (cadar exp)) (flat_seq_helper2 (cdr exp)) ))

    	(else 
    		`(,(flat_seq_helper2 (car exp)) ,@(flat_seq_helper2 (cdr exp)))))))
